...
mmddyyyy extract_date(char *date)
{
	...
	if(!construct_mmddyyyy(sd, d, m, y))
		throw (string)("invalid date (" + date_to_string(m, d, y) + ") ...");
	return sd;
}
...
bool valid_date(int d, int m, int y)
{
	if(m==1 || m==3 || m==5 || m==7 || m==8 || m==10 || m==12)
		...
		return (is_leap_year(y)) ? (d>=1 && d<=29) : (d>=1 && d<=28);
	else
		return false;		
}
...
string date_to_string(UI mm, UI dd, UI yyyy)
{
	char str[100];
	sprintf(str, "%u\\%u\\%u", mm, dd, yyyy);
	return (string)(str);
}
