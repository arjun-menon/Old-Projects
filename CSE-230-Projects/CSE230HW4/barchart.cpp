...
void star_adjust(map<mmyyyy, UI> &k)
{
	/*
	...
	for(map<mmyyyy, UI>::const_iterator i = k.begin(); i!=k.end(); i++)
		if(i->second > max)
			max = i->second;

	if(max > MAX_LINE_SIZE)
		for(map<mmyyyy, UI>::iterator it = k.begin(); it!=k.end(); it++)
			it->second = (it->second * MAX_LINE_SIZE) / max;
}
