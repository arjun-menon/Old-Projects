/*******************************************
  ...
  Author: Arjun Menon  |  Date: 6/09/05
*******************************************/
...
bool eval(const char *str, char *err, register struct data *eval)
{
	...
	for(;*str!='\0';str++)
	{
		/* we find the no. of brackets */
		if(*str=='(')
		brackets[0]+=1;
		if(*str==')')
		brackets[1]+=1;
	}
	/* check if the equation is mathematically correct (bracket count) */
	if(brackets[0]+brackets[1]==0)
	{
		/* this is the simplest deduction of the equation */
		printf("1");
		return false;
	}
	else if(brackets[0]==brackets[1])
	{
		/* this is the complex form of the equation */
		printf("2");
		return false;
	}
	else  /* the expression is mathematically WRONG. */
	{
