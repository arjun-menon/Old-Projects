/*******************************************
  ...
  Author: Arjun Menon  |  Date: 22/10/05
*******************************************/
...
PROTOTYPE(char num_check, (const char *str));
...
void eval(const char *str, char *err, long *val)
{
	...
	else
	{
		...
		else if(*str=='\\')  /* the expression is postfix */
		{
			strcpy(trans,str);
		}
		else  /* it's an ordinary infix expression */
		{
			/* it's an ordinary infix expression */
			PROTOTYPE(void iftopf, (const char *str, char *err, char *pfe));
			iftopf(str,err,trans); /* covert to postfix */
			if(strlen(err))
			return;  // <--------------- FUNCTION RETURN
		}
		/*
		 ...
		 */
		{
			...
			for(lc=0;;lc++)
			{
				if(trans[lc]>='0'&&trans[lc]<='9')
				stack_size = stack_size*10 + (trans[lc]-'0');
				else if(trans[lc]==':') break;
				else if(trans[lc]=='\0') {  /* unspecified stack size */
			...
			int *stack = NULL;
			if((stack=(int *)(malloc(sizeof(int)*
			(stack_size))))==NULL) allocerr();
			int stlc = 0; /* stack counter */

			/* this is string is used to temporarily 
			 store elements of the postfix expression */
			char *ops = NULL;
		...
		{
			strcpy(err,"Fatal Expression Evaluation Error");
			return;  // <--------------- FUNCTION RETURN
		}
	}
}

void iftopf(const char *str, char *err, char *pfe)
{
...
char num_check(const char *str)
{
	...
	// return>0 when a a non-numerical is found & vice-versa.
