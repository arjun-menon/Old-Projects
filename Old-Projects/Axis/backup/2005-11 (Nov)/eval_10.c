...
void * allocs(unsigned long int len);
...
void exec(char *str)
{
	const long int strn = strlen(str);
	char *lhs = (char *) allocs(strn);
	char *rhs = (char *) allocs(strn);
	char *sptr = NULL;
...
void eval(const char *str, long int *value)
{
	...
	char ostack[32]={'+','*','-'};
	long int estack[32]={16,32,64};

	/*
	 *  Array size of *estack will always
	 *  be 1 more than the array size of
	 *  *ostack (becuase 1 operator means
	 *  two values exist to be operated
	 *  upon). The size of *estack is 
	 *  given in int stack_size.
	 */

	// remeber alloc, little mem waste
	// on estack, when ( & ) put in ostack.
