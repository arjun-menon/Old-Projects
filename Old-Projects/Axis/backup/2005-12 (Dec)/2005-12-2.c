/***************************************************
  ...
  Author  Name: Arjun G. Menon  |  Date: 2/12/2005
****************************************************/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
void exec(const char *str, char * const err);
void eval_long(char *str, long int *value, char * const err);
void vals_long(const char *str, long int *value, char * const err);
long int * lex_long(const char *str, long int *lex, char * const err);
/* memory management: */
...
void * lisp_sqrt(int argc, char *argv[]);
...
void runs(const char *str)
{
	...
	reallocstr(and);
	/* err is used as an error descriptor string. */
	char *err = allocs(strlen(and)+STR_MAX);
	/* now execute "and" */
	exec(and,err);
	if(strlen(err))
	msg(err,2);
...
void exec(const char *str, char * const err)
{
	...
	for(;*str!='\0';str++,strn++) {
		...
		strcpy(err,"Invalid Syntax, incorrect number of parentheses.");
		return; }
	str -= strn;
	...
	*sptr = '\0';
	...
	if(*rhs=='\0') { /* <- empty rhs, so no assignment involved. */
		...
		if(*rhs=='>')
			exchange_vars(lhs,rhs,err);
		else { /* variable creation/assignment */
			void asgn(char *rhs, char *lhs, char * const err);
			asgn(rhs,lhs,err);
		}
	}
	/* free memeory */
...
long int * lex_long(const char *str, long int *lex, char * const err)
{
	/*   Rules of Abstract Lexical Analysis
	 *  ------------------------------------
	 ...
	 *  definitions, declarations are NOT accepted.
	 *
	 *  First it analyses a string, and finds
	 *  out it's value, (if there exists one);
	 *  The value will be a long int. If not,
	 *  then it will be typecasted to long int.
	 *
	 *  If the *str describes a pointable matter
	 *  (eg. variable names), then a pointer to
	 *  the variable is returned and the long int
	 *  pointed to by *lex is ignored.
	 *
	 *  If the *str describes volatile matter
	 *  (eg. absolute functions), then it is
