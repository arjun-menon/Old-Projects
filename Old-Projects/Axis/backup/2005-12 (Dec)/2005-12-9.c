/***************************************************
  ...
  Author  Name: Arjun G. Menon  |  Date: 9/12/2005
****************************************************/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
num32 new_idHash(const char *id)
{
	if((get_idHash(id))!=NULL) return NULL;
	...
	/* reallocate table structure with new size */
	if((table_ptr = (struct struct_idHashTable *) 
	realloc (idHashTable, sizeof(struct struct_idHashTable)*
	idHashTable_size))==NULL) msg(system_no_mem,3);
	else /* set the variable */
	...
	/* return a pointer to the variable */
	return idHashTable[int32_table_size-1].key;
}
