/***************************************************
  ...
  Author  Name: Arjun G. Menon  |  Date: 1/1/2006
****************************************************/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
double * new_dbl(const char *id)
{
	/* chekc if variable already exists */
	if(get_dbl(id) != NULL)
		return NULL;
	...
	if(value!=NULL)
	{
		...
		if(value!=NULL)
		{
			...
			* value = rvalue;
			...
			lisp_error = allocs(32+strlen(lhs));
			sprintf(lisp_error,"LHS Variable '%s' does not exist.",lhs);
		}
	}
	else {
		lisp_error = allocs(32+strlen(rhs));
		sprintf(lisp_error,"RHS Variable \"%s\" does not exist.",rhs);
	}
}
...
void exec(const char *str)
{
	...
	/* check validity of equation in number of brackets */
	{
		...
		if(brackets!=0)
		{ lisp_error = clones("Invalid Syntax, incorrect number of parentheses.");
		return; }
		str -= strn;
	}
...
void asgn(char *rhs, char *lhs)
{
	...
	if(!isvalid_id(lhs))
		...
		lisp_error = clones("Illegal Identifier Syntax.");
		return;
	}
	...
	else lisp_error = clones("Fatal Data Transfer Error.");
}
	...
	if(list)
	{
		if(n == 1) /* extract variable */
		{
			...
			else
			{
				*value = 0;
				free_list(list,n);
				lisp_error = allocs(40+strlen(str));
				sprintf(lisp_error,"Variable \"%s\" does not exist.",str);
				return get_dbl(str);
			}
		}
		else /* call function */
		...
		lisp_error = allocs(20+strlen(str));
		sprintf(lisp_error,"Invalid Synatax: ",str);
		return NULL;
	}
}
