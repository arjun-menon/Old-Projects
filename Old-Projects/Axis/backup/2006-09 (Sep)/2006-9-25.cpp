/***************************************************
   ...
   |    Release Date: 2006 September 25          |
****************************************************/
...
num32 exec(const char *str, byte direct);
...
num32 exec(const char *str, byte direct)
{
	...
	static num32 iterator = 0;
	if(iterator > 0 && !direct) {
		//printf("\nWill do \"%s\" %u times.\n",str,iterator);
	...
	if(*rhs=='\0') /* <- empty rhs, so no assignment involved. */
	{
		...
		if(value==NULL) {
			...
			*value = val;
			...
			/* checks for anti-iterator */
			if(lhs[strlen(lhs)-1]!=';')
			iterator = (num32)(abs((int32)(floor(val))));
