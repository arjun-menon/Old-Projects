...
double * new_dbl(const char *id, double_list *dbl_list, num32 size);
double * get_dbl(const char *id, double_list *dbl_list, num32 index);
...
double * new_dbl(const char *id, double_list *dbl_list, num32 size)
{
	...
	if(get_dbl(id, &glob_dbl_list, 0) != NULL)
		return NULL;
...
void exchange_vars(char *lhs, char *rhs)
{
	...
	/* now exchange */
	double *value = get_dbl(rhs, &glob_dbl_list, 0);
	if(value!=NULL)
	{
		/* okay, rhs is valid */
		double rvalue = *value;
		value = get_dbl(lhs, &glob_dbl_list, 0);
		if(value!=NULL)
		{
			/* now swap the values */
			* get_dbl(rhs, &glob_dbl_list, 0) = * value;
			* value = rvalue;
			if(debug_mode)
			  printf("%s => %s\n",lhs,rhs);
		}
...
void run_line(const char *str)
{
	...
	/* now execute code */
	ai_error = 0;
	num32 next_line = exec_line(code, 0);
...
num32 exec_line(const char *str, byte direct)
{
	...
	else if(!strlft(tmp,str,5) && !stricmp(tmp,"loop "))
	{
		...
		double val=0;
		eval_dbl(tmp,&val);
		loop = (num32)(abs((int32)(floor(val))));
	...
	if(*rhs=='\0') /* <- empty rhs, so no assignment involved. */
	{
		...
		if(value==NULL) {
			...
			double val=0;
			eval_dbl(lhs,&val);
			value = get_dbl(0, &glob_dbl_list, 0);
			*value = val;
...
void asgn(char *rhs, char *lhs)
{
	...
	if(rhs_args == lhs_args)
	{
		...
		for(; i<lhs_args ; i++)
		{
			...
			if(!isvalid_id(lhs_list[i]))
				/* create the variable */
				value = new_dbl(lhs_list[i], &glob_dbl_list, 1);
			else {
			...
			if(value==NULL)
			value = get_dbl(lhs_list[i], &glob_dbl_list, 0);
	...
	else
	{
		...
		if(!isvalid_id(lhs))
			/* create the variable */
			value = new_dbl(lhs, &glob_dbl_list, 1);
		else {
			ai_error = clones("Illegal Identifier Syntax.");
			return;
		}
	...
	if(list)
	{
		if(n == 1) /* extract variable */
		{
			if(!strcmp("_",str))
			{
				*value = *get_dbl(0, &glob_dbl_list, 0);
				free_list(list,n);
				return get_dbl(0, &glob_dbl_list, 0);
			}
			else if(get_dbl(str, &glob_dbl_list, 0) != NULL)
			{
				*value = *get_dbl(str, &glob_dbl_list, 0);
				free_list(list,n);
				return get_dbl(str, &glob_dbl_list, 0);
			}
			else
			{
				*value = 0;
...
{
	double *var_ptr = NULL;
	var_ptr = get_dbl(S1, &glob_dbl_list, 0);
	if(var_ptr == NULL) {
		ai_error = allocs(40+strlen(S1));
		sprintf(ai_error,"Variable \"%s\" does not exist.",S1);
		return 0;
...
void c2h(int argc, char *argv[])
{
	...
	/* user interface */
	printf("%s\n","===========================================");
	printf("%s\n"," << SOURCE CODE TEXT TO HTML TRANSLATOR >> ");
	printf("%s\n","===========================================");
	printf("%s\n","  This program converts C source code text "); 
	printf("%s\n","  file to an HTML page with line numbers.  ");
	printf("%s\n","  This program was written by Arjun Menon. ");
	printf("%s\n","-------------------------------------------");
	printf("%s\n","  To modify the default color settings,    ");
	printf("%s\n","  precede the filename with : colon symbol ");
	printf("%s\n","  for monochrome, or ? for user-defined.   ");
	printf("%s\n","===========================================");

	if(argc<=1)  /* the are no arguments */
	{
		printf("%s\n","Please enter the name of the file to translate...");
...
void c2html(const char *filename, char *err, struct colors *c)
{
	...
	fprintf(hfile,"%s","</CODE><HR NOSHADE><FONT FACE=Arial>\n");
	...
	fprintf(hfile,"%s","</FONT></BODY></HTML>");
...
void eas()
{
    ...
    printf("%s\n"," ======================== ");
    printf("%s\n"," This program searches random data and extracts ");
    printf("%s\n"," e-mail addresses out of them if present. \n    "); 
...
void f2c()
{
    ...
    printf("%s\n","===========================================");
    printf("%s\n"," <<  FILE  TO  C  LANGAUGE  TRANSLATOR  >> ");
    printf("%s\n","===========================================");
    printf("%s\n","  This program converts any ordinary file  "); 
    printf("%s\n","  into a C program that generates the file ");
    printf("%s\n","  from an array made of the file's bytes.  ");
    printf("%s\n","  This program was written by Arjun Menon. ");
    printf("%s\n","-------------------------------------------");
    printf("%s\n","   U S A G E    I N S T R U C T I O N S :  ");
    printf("%s\n"," Please enter the name of the file to translate");
    printf("%s\n"," and target filename. To use same target filename, ");
    printf("%s\n"," leave blank; otherwise enter the target name...");
    printf("%s\n"," By default, the array is written in hexadecimal ");
    printf("%s\n"," with 16 numbers on one line of code, to modify ");
    printf("%s\n"," these settings precede the source filename with ");
