...
void resize_dbl(const char *id, double_list *dbl_list, 
				num32 new_size);
...
char strsim(const char *str, const char *cmp);
...
num32 invoke_keywords(const char *str, int32 *loop, byte *cont)
{
	...
	*cont = 0;
	...
	if(!strlft(tmp,str,4) && !strsim(tmp,"term"))
	{
		terminate();
		initialize();
	...
	else if(!strlft(tmp,str,6) && !strsim(tmp,"write "))
	{
		...
		if(!strsim(tmp,"nl"))
			puts("\n");
		else if(!strsim(tmp,"newline"))
			puts("\n");
		else if(!strsim(tmp,"prog"))
			prog_show(stdout);
		else if(!strsim(tmp,"dump"))
			show_dbl_list(&glob_dbl_list, stdout);
		else if(*tmp == '\"') {
			trim(tmp,'\"');
			puts(tmp);
	...
	else if(!strlft(tmp,str,5) && !strsim(tmp,"loop "))
	{
		postchar(tmp,str," ");
...
double call_func(char ** list, num32 args)
{
	...
	#define FN_CALL(FN) else if(!strsim(list[0],FN)) return
	#define PR_CALL(FN) else if(!strsim(list[0],FN))
	...
	#undef IFFN
		...
		ai_error = allocs(strlen(list[0]+100));
		sprintf(ai_error,
