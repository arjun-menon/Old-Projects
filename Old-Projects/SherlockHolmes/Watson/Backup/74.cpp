...
void DecipherString(char *String);
...
{
	/* the sole purpose of the encryption
	   used above is to make the information 
	   illegible in the compiled object file.
	*/
	DecipherString(LocLogFileExt);
	DecipherString(LogCountFile);
	DecipherString(SrvLogFileExt);
	DecipherString(UplinkCountFile);
	DecipherString(LocIdFile);
	DecipherString(SrvIdFile);
	DecipherString(InternetCheckMask);
	DecipherString(FtpServer);
	DecipherString(FtpUserName);
	DecipherString(FtpPassword);
}
			...
			NULL, FTP_TRANSFER_TYPE_BINARY, 0))
		{
			...
			if((idf = fopen(idf_name,"r+b")) != NULL)
			{
				...
				// Got the old ID number
				{
					...
					else /* can't re-open local ID file 
						    to write the new ID number */
						return cid;
				}
