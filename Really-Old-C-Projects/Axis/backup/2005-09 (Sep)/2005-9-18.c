/*******************************************
  ...
  Author: Arjun Menon  |  Date: 18/09/05
*******************************************/
...
void evali(const char *str, char *err, int *eval)
{
	...
	void valstr(char *vals, const char *str);
...
void valstr(char *vals, const char *str)
{
	const char nox[] = "Nexus 1.0";
	// NOT COMPLETED ----------------------------
	if((cp=(char *)(realloc(vals,sizeof(char)*(strlen(nox)+1))))!=NULL)
	{ vals = cp; } else { allocerr(); }
	strcpy(vals,nox);
}
