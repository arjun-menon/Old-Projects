/*******************************************
  ...
  Author: Arjun Menon  |  Date: 8/09/05
*******************************************/
...
inline bool ncheck(const char *str);
...
bool eval(const char *str, char *err, register struct data *eval)
{
	...
	if(!bcheck(str))
	{
		...
		char *tmp = new char[strlen(str)+1]; *tmp='\0';
		if(prechar(tmp,str,symbols)==0)
		{printf("\nultima");}
		delete tmp;

		/* now the real maths */
		char *ex = new char[strlen(str)+1]; *ex='\0';
		char *ps = new char[strlen(str)+1]; *ps='\0';
		char *px = new char[strlen(str)+1]; *px='\0';
...
inline bool ncheck(const char *str)
{
	// checks wether a string is amde up of purely numbers.
	for(;*str!='\0';str++)
	if(*str<='0'&&*str>='9')
	return true;
	return false;
