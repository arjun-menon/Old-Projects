/*******************************************
  ...
  Author: Arjun Menon  |  Date: 19/10/05
*******************************************/
...
void eval(const char *str, char *err, long *val)
{
	...
	else
	{
		...
		else if(*str!='\\')  /* the expression is not postfix */
		{
		 ...
		 *  in the string trans. Otherwise, if it
		 *  was an accelerated expression, then it
		 *  has been solved to *val and returned
		...
		if(*trans=='\\')
		/*
		 *  In order to ensure perfection, each postfix
		 *  expresion *must* begin with a '\' character
		 */
		{
			//
		}
