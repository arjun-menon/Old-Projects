/*******************************************
  ...
  Author: Arjun Menon  |  Date: 24/10/05
*******************************************/
	...
	else
	{
		 ...
		 */
			...
			for(lc=0;;lc++)
			{
				...
				else if(str[lc]==':') { strcpy(trans,&str[lc+1]);
				if(!stack_size) stack_size = 32; break; }
		...
		char oplc = 0;  /* char ops[] counter */

		for(;;lc++,oplc++)
		{
			if(trans[lc]!=','&&trans[lc]!='\0')
			{
				ops[oplc] = trans[lc];
			}
			else  /* trans[lc] == ',' or '\0' */
