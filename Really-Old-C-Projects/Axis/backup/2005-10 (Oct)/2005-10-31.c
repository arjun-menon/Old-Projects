/*******************************************
  ...
  Author: Arjun Menon  |  Date: 31/10/05
*******************************************/
...
const char eval_version[] = "EVAL 0.0.1";
...
{
	// This function translate a number
...
{
	// this function translates a float
	// into a string & stores it in *str.
...
void doublestr(char *str, const double f)
{
	...
	/*  this function is almost
	 *  identical to the previous
	 *  function except that the
	 *  data types of freal & fint
...
{
	// check if an equation has the
	// corrext number of brackets.
...
int iftopf(const char *str, char *err, char *pfe)
{
	...
	int prior(char e);
	void push(char *stack, int *sp, char item);
	...
	for(;len&&s;len--)
	{
		/* skip off tabs and spaces */
		if(*s==' '||*s=='\t')
		{
			/* go to next char in s */
			s++;
			continue;
		}

		/* identiry number & aplhabets*/
		if((*s>='0'&&*s<='9')||
		   (*s>='A'&&*s<='Z')||
		   (*s>='a'&&*s<='z'))
		{
