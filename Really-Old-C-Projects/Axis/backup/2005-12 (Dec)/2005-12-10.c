/***************************************************
  ...
  Author  Name: Arjun G. Menon  |  Date: 10/12/2005
****************************************************/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
inline void init_int32(struct int32_list * int32_offset);
...
{
	int32 *v;
	v = mynewint32("aa"); *v = 49; printf(" => %ld\n",*v); puts("");
	v = mynewint32("bb"); *v = 56; printf(" => %ld\n",*v); puts("");
	v = mynewint32("cc"); *v = 78; printf(" => %ld\n",*v); puts("");
	v = mynewint32("dd"); *v = 99; printf(" => %ld\n",*v); puts("");
	v = mygetint32("cc"); printf(" => %ld\n",v!=NULL?v:0); puts("");
...
void eval_int32(char *str, int32 *value)
{
	...
	for(cur=0,top=0,ival=0;cur<ops;cur++)
	{
		...
		case 'E': /* == */ stack[--ival-1] = stack[ival]; break;
		case '>': /* >  */ stack[--ival-1] = stack[ival]; break;
		case 'F': /* >= */ stack[--ival-1] = stack[ival]; break;
		case '<': /* <  */ stack[--ival-1] = stack[ival]; break;
		case 'G': /* <= */ stack[--ival-1] = stack[ival]; break;
...
int32 * mynewint32(const char *id)
{
	...
	printf("made: %ld for %s\n",address,id);
	if(!address)
		return NULL;
	printf("int %s -> %ld",id,address);
...
num32 new_idHash(const char *id)
{
	printf("new called at(1): %lu\n",idHashTable);
	if((get_idHash(id)))
		return 0;
	printf("new called at(2): %lu\n",idHashTable);
	printf("so making %s\n",id);

	/* increment table size by 1 for 1 new variable */
	idHashTable_size += 1;	
	printf("gonna hash id no:%d \n",idHashTable_size);

	/* reallocate table structure with new size */
	struct struct_idHashTable * table_ptr;
	table_ptr = (struct struct_idHashTable *) realloc
	(idHashTable, sizeof(struct struct_idHashTable)*idHashTable_size);

	printf("hashed !!! id no:%d \n",idHashTable_size);
	if(table_ptr == NULL)
		msg(system_no_mem,3);
	else /* set the variable */
		idHashTable = table_ptr;
	printf("so, hashed id no:%d \n",idHashTable_size);
...
num32 get_idHash(const char *id)
{
	...
	if(!idHashTable_size) {
		puts("never there"); return NULL; }

	/* find out the location of the variable */
	register num32 index;
	for(index = 0;index < idHashTable_size; index++)
		if(!strcmp(idHashTable++ ->str, id)) break;
	idHashTable -= index + 1;

	/* check if the variable exists */
	if(index >= idHashTable_size)
	{ puts("not there");idHashTable++; return 0; }

	printf("found %s = %ld\n",id,idHashTable[index].key);
	/* return a pointer to the variable */
...
inline void init_int32(struct int32_list * int32_offset)
{
	int32_offset ->next = NULL;
	int32_offset ->value = NULL;
	int32_offset ->id_num = 0;
	int32_offset ->array_size = 0;
}
