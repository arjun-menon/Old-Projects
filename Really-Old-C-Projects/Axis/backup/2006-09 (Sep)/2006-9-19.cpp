/***************************************************
   ...
   |    Release Date: 2006 September 19          |
****************************************************/
...
num32 store_line(char *str);
const char * fetch_line(num32 lno);
...
num32 store_line(char *str)
{
	const num32 str_len = strlen(str);
	if(!str_len || *str == '\n')
		return 0;

	prog_table_size++;
	prog_table = (LIST) realloc(prog_table, 
		prog_table_size * sizeof(char *));

	prog_table[prog_table_size-1] = allocs(str_len);
	strcpy(prog_table[prog_table_size-1], str);

	return prog_table_size;
}

const char * fetch_line(num32 lno)
{
	if(lno <= prog_table_size)
		return prog_table[lno-1];
	else return 0;
}
...
void exec(const char *str)
{
	...
	else if(!strlft(tmp,str,6))
		if(!stricmp(tmp,"write "))
	{
		postchar(tmp,str," ");
		puts(tmp);
...
double call_func(char ** list, num32 args)
{
	...
	PR_CALL("f2c")   { puts(""); f2c();    }
	PR_CALL("c2h")   { puts(""); c2h(0,0); }
	PR_CALL("eas")   { puts(""); eas();    }
