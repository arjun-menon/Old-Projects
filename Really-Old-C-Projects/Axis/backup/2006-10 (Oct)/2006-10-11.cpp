...
double * get_dbl(const char *id, double_list *dbl_list)
{
	...
	num32 index = lex_subscript(id, var_id);
...
void allocerr() { msg(system_no_mem, 4); }
...
//		(strlen(pre)+1))))==NULL) allocerr(); 
		ax = allocs(strlen(pre)+1);
		*ax='\0';

//		if((bx=(char *)(malloc(sizeof(char)*
//		(strlen(pos)+1))))==NULL) allocerr(); 
