...
num32 invoke_keywords(const char *str, int32 *loop, byte *cont)
{
	...
	*cont = 0;
	...
	else if(!strlft(tmp,str,4) && !stricmp(tmp,"del "))
	{
		postchar(tmp,str," ");
	...
	else if(!strlft(tmp,str,5) && !stricmp(tmp,"dump "))
	{
		postchar(tmp,str," ");
	...
	else if(!strlft(tmp,str,4) && !stricmp(tmp,"dump"))
	{
		puts("\nCore dump: ");
	...
	else if(!strlft(tmp,str,6) && !stricmp(tmp,"debug "))
	{
		postchar(tmp,str," ");
