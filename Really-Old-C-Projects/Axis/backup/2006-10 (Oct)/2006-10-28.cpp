...
void x()
{
	hash_table_struct *h = NULL;
	num32 s = 0;
	int32 *t = NULL;

	t = hash_newkey("ff",&h,&s);
	*t = 33;
	printf("[%u] %x -> %d\n",s,t,*t);
	t = hash_newkey("gg",&h,&s);
	*t = 99;
	printf("[%u] %x -> %d\n",s,t,*t);
	t = hash_newkey("kk",&h,&s);
	*t = 222;
	printf("[%u] %x -> %d\n",s,t,*t);

	puts("");
	t = hash_getkey("gg",&h,&s);
	if(t) printf("[%u] %x -> %d\n",s,t,*t);
	t = hash_getkey("kk",&h,&s);
	if(t) printf("[%u] %x -> %d\n",s,t,*t);
	t = hash_getkey("ff",&h,&s);
	if(t) printf("[%u] %x -> %d\n",s,t,*t);
}
...
inline void free_goto_table()
{
	free_str_array(&goto_table, goto_table_size);
}
...
void ws()
{
    ...
    count = word_scan(sfile, dfile, '@', err);
