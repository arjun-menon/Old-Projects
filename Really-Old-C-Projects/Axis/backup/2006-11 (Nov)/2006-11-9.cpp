...
double * new_dbl_var(const char *var_id, 
	double_list *dbl_list, num32 size);
double * get_dbl_var(const char *var_id, 
	double_list *dbl_list, int32 index);
void resize_dbl_var(const char *id, 
	double_list *dbl_list, num32 new_size);
char del_dbl_var(const char *id, double_list *dbl_list);
void mov_dbl_str(const char *id, 
	double_list *dbl_list, const char *str);
void show_dbl_str(const char *id, double_list *dbl_list, FILE *buf);
void show_dbl_list(double_list *dbl_list, FILE *buf);
...
void create_thread();
void destroy_thread(num32 thread);
void free_prog_table_all();
void free_goto_table_all();
inline byte prog_write(num32 thread, char *str);
inline const char * prog_read(num32 thread, num32 index);
inline void prog_show(num32 thread, FILE *buf);
inline void free_prog_table(num32 thread);
void set_goto_line(const char *str, num32 line);
num32 get_goto_line(const char *str);
inline void free_goto_table();
void run_line(const char *str);
num32 exec_line(num32 thread, const char *str, byte direct);
void asgn(char *rhs, char *lhs);
inline void mov_dbl(double *value, 
...
/* language executives */
num32 invoke_keywords(num32 thread, 
const char *str, int32 *loop, byte *cont);
void operation_print(num32 thread, char *str, char newline);
inline void operation_term();
void operation_del(char *str);
inline void operation_dump();
void operation_dumpf(char *str);
void operation_debug(char *str);
inline void operation_debugm();
inline void operation_pause();
void operation_exec(char *str);
void operation_write(char *str);
void operation_repeat(char *str, int32 *loop);
void operation_progl(num32 thread, char *str);
inline void operation_prog(num32 thread);
void operation_file(char *str);
void operation_release();
void operation_store(num32 thread, char *str);
inline void operation_exit();
...
inline void strip_quotes(char *str);
...
inline char strisc(const char *str, const char chr);
inline num32 charcount(const char *str, const char ch);
inline void strbluff(char *str, const char *bluff, const num32 n);
inline void strnully(char *str, const num32 n);
inline void strnull(char *str);

/* string allocation: */
inline char * allocs(num32 len);
inline char * reallocstr(char *str);
...
double * new_dbl_var(const char *var_id, double_list *dbl_list, num32 size)
{
...
double * get_dbl_var(const char *var_id, double_list *dbl_list, int32 index)
{
...
void resize_dbl_var(const char *id, double_list *dbl_list, 
					num32 new_size)
{
...
void mov_dbl_str(const char *id, double_list *dbl_list, const char *str)
{
...
void store_str(char *str, LIST* str_array, num32 *array_size)
{
	...
	//printf("store_str %s @ %u\n", (*str_array)[(*array_size)-1],
	//	(*str_array)[(*array_size)-1]);
}
...
void create_thread()
{
	...
	prog_table = (LIST *) realloc(prog_table,
		(thread_count * sizeof(LIST)));
	if(prog_table == NULL)
		msg(system_no_mem, 4);

	prog_table[thread_count-1] = NULL;
}
...
inline byte prog_write(num32 thread, char *str)
{
	...
	//printf("write @ %u to %u\n",thread,prog_table[thread]);
	store_str(str, &(prog_table[thread]), &prog_table_size);
	//printf("written(%u): %u -> %s\n", prog_table_size, 
	//(prog_table[thread])[prog_table_size-1],(prog_table[thread])[prog_table_size-1]);
	return 1; /* success */
}
...
inline const char * prog_read(num32 thread, num32 index)
{
	if(index > 0 && index <= prog_table_size) 
		...
		&(prog_table[thread]), prog_table_size); 
	else
		return NULL;
}
...
inline void free_prog_table(num32 thread)
{
	free_str_array(&(prog_table[thread]), prog_table_size);
	prog_table = NULL;
	prog_table_size = 0;
}
...
void run_line(const char *str)
{
	...
	if(prog_write(active_thread, code))
		linenum++;
	...
	/* now execute code */
	ai_error = NULL;
	next_line = exec_line(active_thread, code, 0);
...
checkpoint:
	if(next_line)
		if(prog_read(active_thread, next_line))
		/* go to the past */
	{
		...
		while(i <= linenum) {
			...
			//puts(ns);
			ai_error = NULL;
			next_line = exec_line(active_thread, ns, 0);
...
num32 exec_line(num32 thread, const char *str, byte direct)
{
	...
	byte cont = 0;
	int32 ret = invoke_keywords(thread, str, &loop, &cont);
	if(!cont)
		return ret;
...
num32 invoke_keywords(num32 thread, const char *str, int32 *loop, byte *cont)
{
	/*
	...
	*cont = 0;
	...
	else if(!strlft(tmp,str,4) && !strsim(tmp,"eqa "))
	{
		postchar(tmp,str," ");
		eqa(tmp);
	...
	else if(!strlft(tmp,str,5) && !strsim(tmp,"eval "))
	{
		postchar(tmp,str," ");
		eqa(tmp);
...
void operation_progl(num32 thread, char *str)
{
	...
	const char *line = prog_read(thread, line_no);
	printf("\n");
...
/**********************************************
  C SOURCE CODE TO HTML PAGE TRANSLATOR v0.23
***********************************************/
...
void evali(const char *str, char *err, int *eval)
{
	//  Integral Expression Analyzer & Evaluator
	//  ===========================================
	//  This function analytically evaluates simple
	//  infix mathematical expressions. The expression
	//  is split into components based on precedence.
	//  Every component has only one single operator.
	//  and each component is evaluated recursively.
	//  Using this methid complex expressions can be
	//  analyzed and expressed as a combination of
	//  simple components. Each component is presented
	//  in the assembly language syntax for analysis.
...
/********************************
  Charcter-Bounded Word Scanner
*********************************/

void ws()
{
    unsigned int word_scan(char *sfile, char *dfile, char key, char *err);
    
    puts("\n Charcter-Bounded Word Scanner ");
    puts(" ============================= ");
    puts(" This program searches random data and extracts ");
    puts(" specific set of words  surrounded by a single ");
    puts(" user-defined character out of them if present. "); 
    ...
    {
      printf("\n File already exists. Continue (n:no,~n:yes) ? ");
    ...
    count = word_scan(sfile, dfile, key, err);
    
    if(!strlen(err))
      printf("\n Scan Successful. Found %u words.\n",count);
    else
      printf("\n Scan failed. Error: %s\n",err);
...
unsigned int word_scan(char *sfile, char *dfile, char key, char *err)
{
     ...
     {
       char Slft[STR_MAX ], Srht[STR_MAX ], Word[STR_MAX ];
       if(prechar(Slft,str,keys)==key)
       {                        
         postchar(Srht,str,keys);
         ...
         sprintf(Word,"%s%c%s",SlftT,key,Srht);
         count++;
         fprintf(fd,"%s\n",Word);
         puts(Word);
       }
            ...
            for(i=0; i<bytes; i++)
            {
                ...
                if(count<fsize)
                {
                  ...
                  case 'c':
                       ...
                       &&buffer[i]!='\"'
                       &&buffer[i]!='\\')
                         fprintf(cfile,"'%c',",(unsigned char)
											(buffer[i]));
                       else
                         fprintf(cfile,"%u,",(unsigned char)
											(buffer[i]));
                       break;
