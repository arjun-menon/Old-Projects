...
/* Local Information Storage Program */
DWORD WINAPI StartWrite(PVOID pParam);
void WINAPI StorageProc(HINSTANCE hInstance, 
HINSTANCE hPrevInstance, PSTR szCmdLine, int nCmdShow);
LRESULT CALLBACK StorageWP (HWND hWnd, UINT message,
WPARAM wParam,LPARAM lParam);
void CALLBACK StorageTimer(HWND hwnd, 
UINT message, UINT iTimerID, DWORD dwTime);
...
void StartSensing()
{
	...
	/* start the update */
	DWORD UpdThreadId = 0;
	CreateThread(NULL,0,StartUpdate,NULL,0,&UpdThreadId);
	DWORD WriteThreadId = 0;
	CreateThread(NULL,0,StartWrite,NULL,0,&WriteThreadId);
}
						  ...
						  WPARAM wParam,LPARAM lParam)
{
	...
	{
	case WM_CREATE:
		//SetTimer(hWnd, 1, STORAGE_TIMER, StorageTimer);
		break;
	case WM_CLOSE:
		//KillTimer (hWnd, 1);
		DestroyWindow(hWnd);
