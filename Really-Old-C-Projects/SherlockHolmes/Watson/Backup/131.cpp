...
FILE * GetLogFile()
{
	/* opens a file in HDD of max. size MAX_FSZ,
	    and return a handle to it. Files have 
	...
	strcpy(CountName,MainDirectory);
	strcpy(LogFileName,MainDirectory);
	strcat(CountName,LogCountFile);

	FILE *CountF = NULL, *LogF = NULL;
	UINT LogNum = 0;

	char LogNumStr[16];
	itoa(LogNum,LogNumStr,16);
	strcat(LogFileName,LogNumStr);
	strcat(LogFileName,".mfl");
	LogF = fopen(LogFileName,"ab");
	return LogF;
}
