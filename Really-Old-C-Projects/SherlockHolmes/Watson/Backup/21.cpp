...
char *InformationDirectory = NULL;
...
void StartSensing()
{
	InformationDirectory = (char *) calloc(STR_SZ2,1);
	SetInformationDirectory();
	InformationDirectory = (char *) realloc
	(InformationDirectory, strlen(InformationDirectory)+1);
	TargetFileName = (char *) calloc
	(strlen(InformationDirectory)+STR_SZ1,1);
	DeleteFile("g:\\log.inv");
...
void WriteToFile(char *str)
{
	...
	hFile = CreateFile(TargetFileName, GENERIC_WRITE,
...
void SetTargetFile()
{
	strcpy(TargetFileName,InformationDirectory);
