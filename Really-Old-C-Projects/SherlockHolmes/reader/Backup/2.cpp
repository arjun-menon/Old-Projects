...
void SimpleDecrypt(char *str, const unsigned long int n, const char *key);
char Password[] = "a";
			 ...
			 unsigned long int *newlen)
{
	...
	SimpleDecrypt(buff,strlen(buff),Password);
...
void SimpleDecrypt(char *str, const unsigned long int n, const char *key)
{
	...
	for(i=0;i<n;i++) str[i] = ~(char)(str[i]);
}
