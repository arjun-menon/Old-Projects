...
def construct_zones():
    ...
    for i in range(1, 6+1):
        zones.append(list(map((lambda j: elem(i, j)), range(1, 6+1))))
    # Second 6 zones - the columns
    for j in range(1, 6+1):
        zones.append(list(map((lambda i: elem(i, j)), range(1, 6+1))))
